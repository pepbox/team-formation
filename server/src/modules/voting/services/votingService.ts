...
class VotingService {
    ...
    async createVotingTimers(sessionId: string, votingDuration: number) {
        try {
            const teams = await this.teamService.getTeamsBySessionId(sessionId);
            const votingEndTime = new Date(Date.now() + votingDuration * 1000);
            ...
            console.log(`Created ${timers.length} voting timers for session ${sessionId}`);
        } catch (error) {
    ...
    async processTeamLeaderSelection(teamId: string, sessionId: string) {
        try {
            ...
            if (!teamPlayers || teamPlayers.length === 0) {
                console.log(`No players found for team ${teamId}, skipping leader selection`);
                return;
            }
            ...
            const teamPlayersWithVotes = teamPlayers.map((teamPlayer) => {
                ...
                const playerObj = (teamPlayer as any).toObject ? (teamPlayer as any).toObject() : teamPlayer;
            ...
            if (winners.length === 0) {
                console.log(`No winners found for team ${teamId}, skipping leader selection`);
                return;
            }
            ...
            if (!chosenLeader) {
                console.log(`No leader could be chosen for team ${teamId}, skipping leader selection`);
                return;
            }
            ...
            console.log(`Leader selected for team ${teamId}: ${chosenLeader.firstName} ${chosenLeader.lastName}`);
            ...
            console.error(`Error processing leader selection for team ${teamId}:`, error);
            throw error;
        }
    }
    ...
    private async checkAndUpdateSessionState(sessionId: string) {
        try {
            ...
            if (allTeamsHaveLeaders) {
                ...
                const { default: SessionService } = await import("../../session/services/sessionService");
